...
try {
    $roleNameParts = $GITHUB_ROLE -split "/"
    $roleName = $roleNameParts[-1]
    aws iam get-role --role-name $roleName | ConvertFrom-Json | Select-Object -ExpandProperty Role | Select-Object RoleName, Arn, CreateDate
} catch {
